<?php
namespace Fortifi\FontAwesome\Interfaces\Icons;

interface SportsIcons
{
  const BASEBALL_BALL = 'fa-baseball-ball';
  const BASKETBALL_BALL = 'fa-basketball-ball';
  const BIKING_SPO = 'fa-biking';
  const BOWLING_BALL = 'fa-bowling-ball';
  const DUMBBELL_SPO = 'fa-dumbbell';
  const FOOTBALL_BALL_SPO = 'fa-football-ball';
  const FUTBOL_SPO = 'fa-futbol';
  const GOLF_BALL = 'fa-golf-ball';
  const HOCKEY_PUCK = 'fa-hockey-puck';
  const QUIDDITCH = 'fa-quidditch';
  const RUNNING_SPO = 'fa-running';
  const SKATING_SPO = 'fa-skating';
  const SKIING_SPO = 'fa-skiing';
  const SKIING_NORDIC_SPO = 'fa-skiing-nordic';
  const SNOWBOARDING_SPO = 'fa-snowboarding';
  const SWIMMER_SPO = 'fa-swimmer';
  const TABLE_TENNIS = 'fa-table-tennis';
  const VOLLEYBALL_BALL = 'fa-volleyball-ball';
}
